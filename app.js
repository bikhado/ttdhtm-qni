...
function updateStats() {
    ...
    const totalStudents = filteredData.reduce((sum, s) => sum + s.students, 0);
    const totalTeachers = filteredData.reduce((sum, s) => sum + s.teachers, 0);
    const standardSchools = filteredData.filter(s => s.is_standard).length;
    
    const totalG1 = filteredData.reduce((sum, s) => sum + s.g1_students, 0);
    const totalClasses = filteredData.reduce((sum, s) => sum + s.classes, 0);
    ...
    document.getElementById('stat-total-g1').innerText = totalG1.toLocaleString();
...
function renderTable() {
    ...
    tbody.innerHTML = filteredData.slice(0, 50).map(s => `
        <tr>
            ...
            <td>${s.classes}</td>
            <td>${s.students}</td>
            <td><strong>${s.g1_students || s.g6_students || s.g10_students}</strong></td>
